...
#Date: April 2016
#Version: 1.2 (FactA @ Evalita2016)

#synopsis: perl scorer_factuality.pl gold_folder/ sys_folder/ 
...
for my $fileG (@files_gold){
	...
	if(@refelts > 2){
	    ...
	    if ((($refelts[$col_event] eq "B-EVENT" && $syselts[$col_event] =~ /EVENT/) || ($syselts[$col_event] eq "B-EVENT" && $refelts[$col_event] =~ /EVENT/)) && ($refelts[$col_fv] ne "O")){
		...
		if($match || !$strict_match){
		    ...
		    #FV
		    if ($refelts[$col_fv] eq $syselts[$col_fv]){
			$att_tp{"fv"} += 1;
			if (not(exists($classes{"FV_".$refelts[$col_fv]}))){
			    $classes{"FV_".$refelts[$col_fv]} = 0;
			}
			$classes{"FV_".$refelts[$col_fv]} += 1;
		    }
		    elsif ($refelts[$col_fv] eq "O"){
			$att_fp{"fv"} += 1;
		    }
		    elsif ($refelts[$col_fv] ne $syselts[$col_fv]){
			$att_fc{"fv"} += 1;
			if (not(exists($classes_fc{"FV_".$refelts[$col_fv]}))){
			    $classes_fc{"FV_".$refelts[$col_fv]} = 0;
			}
			$classes_fc{"FV_".$refelts[$col_fv]} += 1;

			#FP
			if (not(exists($classes_fp{"FV_".$syselts[$col_fv]}))){
			    $classes_fp{"FV_".$syselts[$col_fv]} = 0;
			}
			$classes_fp{"FV_".$syselts[$col_fv]} += 1;

			#FN
			if (not(exists($classes_fn{"FV_".$refelts[$col_fv]}))){
			    $classes_fn{"FV_".$refelts[$col_fv]} = 0;
			}
			$classes_fn{"FV_".$refelts[$col_fv]} += 1;
		    }
...
print "FV, accuracy (".$att_tp{"fv"}."/".$nb_ev."): ".$recall_fv."\n";
